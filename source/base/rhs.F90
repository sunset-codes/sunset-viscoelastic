module rhs
  ...
  private
  public calc_all_rhs,filter_variables,calc_all_rhs_c,filter_variables_c
...
!! ------------------------------------------------------------------------------------------------
  subroutine calc_all_rhs_c
     ...
     allocate(gradro(npfb,ithree));gradro=zero  
     allocate(gradu(npfb,ithree),gradv(npfb,ithree),gradw(npfb,ithree));gradw=zero
     call calc_gradient_c(ro,gradro)     
     ...
     !! these subroutines
     call calc_rhs_ro
     call calc_rhs_rovel_c
#ifndef newt     
     !! Only call conformation RHS if non-Newtonian
     call calc_rhs_cholesky_c
#endif
     ...
     return
  end subroutine calc_all_rhs_c
!!-------------------------------------------------------------------------
  subroutine calc_rhs_rovel_c
     !! Construct the RHS for u-equation
     integer(ikind) :: i,j
     real(rkind),dimension(ithree) :: tmp_vec
     real(rkind) :: tmp_scal_u,tmp_scal_v,tmp_scal_w,f_visc_u,f_visc_v,f_visc_w
     ...
     !! Calculate spatial derivatives of velocity
     call calc_laplacian_c(u,lapu)
     call calc_laplacian_c(v,lapv)
#ifdef dim3
     call calc_laplacian_c(w,lapw)          
#endif                                   
     ...
     allocate(gradpsixx(npfb,ithree),gradpsixy(npfb,ithree),gradpsiyy(npfb,ithree))
     allocate(gradpsixz(npfb,ithree),gradpsiyz(npfb,ithree),gradpsizz(npfb,ithree));gradpsixz=zero;gradpsiyz=zero
     call calc_gradient_c(psixx,gradpsixx)
     call calc_gradient_c(psixy,gradpsixy)
     call calc_gradient_c(psiyy,gradpsiyy)   
#ifdef fenep         
     call calc_gradient_c(psizz,gradpsizz)          
#endif     
#ifdef dim3
     call calc_gradient_c(psixz,gradpsixz)    
     call calc_gradient_c(psiyz,gradpsiyz)
#ifndef fenep         
     call calc_gradient_c(psizz,gradpsizz)          
#endif          
#else
...
!        body_force_u = body_force_u + (one/Re)*cos(rp(i,2))*(one+Mdiff*beta*Wi)/(one+Mdiff*Wi)
     ...
     return
  end subroutine calc_rhs_rovel_c
!! ------------------------------------------------------------------------------------------------    
  subroutine filter_variables_c
     !! This routine calls the specific filtering routine (within derivatives module) for each
     !! variable - ro,u,v,w,roE,Yspec - and forces certain values on boundaries as required.
     use mpi_transfers
     integer(ikind) :: ispec,i
     ...
     !! Filter density
     call calc_filtered_var_c(ro)
          
     !! Filter velocity components
     call calc_filtered_var_c(rou)
     call calc_filtered_var_c(rov)
#ifdef dim3
     call calc_filtered_var_c(row)
#endif     
...
!     !! For Cholesky & FENE-P, we should converge to the Cholesky-components of c to filter
     ...
     !! Filter log-conformation or cholesky components
     call calc_filtered_var_c(psixx)
     call calc_filtered_var_c(psixy)
     call calc_filtered_var_c(psiyy)     
#ifdef fenep
     call calc_filtered_var_c(psizz)                                          
#endif     
#ifdef dim3          
     call calc_filtered_var_c(psixz)
     call calc_filtered_var_c(psiyz)                
#ifndef fenep     
     call calc_filtered_var_c(psizz)                                          
#endif     
#endif     
...
!     !! Convert back to Cholesky components of fr*c_{ij}
     ...
     return
  end subroutine filter_variables_c
!! ------------------------------------------------------------------------------------------------  
  subroutine calc_rhs_cholesky_c
     !! Construct the RHS Cholesky decomposition of conformation tensor.
     !! N.B. this is only implemented in 2D for now.
     integer(ikind) :: i,j
     real(rkind),dimension(ithree) :: tmp_vec
     ...
     allocate(lapCxz(npfb),lapCyz(npfb));lapCxz=zero;lapCyz=zero
     if(Mdiff.ne.zero) then
        call calc_laplacian_transverse_only_on_bound_c(Cxx,lapCxx)  
        call calc_laplacian_transverse_only_on_bound_c(Cxy,lapCxy)
        call calc_laplacian_transverse_only_on_bound_c(Cyy,lapCyy)   
        call calc_laplacian_transverse_only_on_bound_c(Czz,lapCzz)           
#ifdef dim3
        call calc_laplacian_transverse_only_on_bound_c(Cxz,lapCxz)
        call calc_laplacian_transverse_only_on_bound_c(Cyz,lapCyz)        
#endif        
     endif
...
!        !$omp end parallel do     
     ...
     return
  end subroutine calc_rhs_cholesky_c
!! ------------------------------------------------------------------------------------------------
end module rhs
