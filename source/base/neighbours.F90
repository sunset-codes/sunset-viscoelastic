module neighbours
  ...
  private
  public :: find_neighbours,find_neighbours_c
...
!! ------------------------------------------------------------------------------------------------
  subroutine find_neighbours_c
    ...
    !! Build cells and create cell linked-lists     
    if(.not.allocated(ic_count))then
       call cell_generator_c
       call neighbour_cells_c
    end if

    ! allocate linking lists
   ! if(allocated(ij_count_small)) deallocate(ij_count_small)
    allocate(ij_count_c(npfb),ij_count_small_c(npfb))    
    allocate(ij_link_c(nplink,npfb))
    n_count=np
    ...
    ! put nodes in different cells
    call divide_c(1,np) 
    call neighboring_list_parallel_c
    ...
    call order_neighbours_c              
        
    write(6,*) iproc,"neighbours found"

  end subroutine find_neighbours_c
!! ------------------------------------------------------------------------------------------------
  subroutine cell_generator_c
     real(rkind) :: sizeX,sizeY
  ...
  end subroutine cell_generator_c
!! ------------------------------------------------------------------------------------------------
  subroutine neighbour_cells_c
    integer(ikind) :: ic,icx,icy,jc,nsheet
    ...
    return
  end subroutine neighbour_cells_c 
!! ------------------------------------------------------------------------------------------------
  subroutine divide_c(nini,nend) 
    !! This subroutine divides the nodes amongst the cells, and creates
    !! a list of particles ordered by their cell
    integer(ikind), intent(in) :: nini,nend 
    real(rkind) :: ddx,ddy
  ...
  end subroutine divide_c
!! ------------------------------------------------------------------------------------------------
  subroutine  neighboring_list_parallel_c
    !! This subroutine loops through every node i, finds the cell ic containing
    !! node i, then goes through every cell jc adjacent to ic to check for neighbours  
    integer(ikind) :: i,ic
    integer(ikind) :: jc,kc
    ...
    ij_count_c=0
    ij_link_c=0 !neighbour list arrays
    ...
    do i=1,npfb 
       ...
       do kc=1,ic_count(ic)
          ...
          !! Add any neighbours in cell jc to neighbour list of particle i
          call neighbour_node_cell_c(i,rpi,jc)
   
       end do      
       
    end do
    !$omp end parallel do
  end subroutine neighboring_list_parallel_c
!! ------------------------------------------------------------------------------------------------
  subroutine neighbour_node_cell_c(ii,ri,jc)
    !! looks through nodes in cell jc and checks whether they are neighbours of 
    !! node ii which has position (vector) ri.
    integer(ikind),intent(in) :: ii,jc !! jc is cell
    real(rkind),dimension(:),intent(in) :: ri
    ...
    if(nc(jc).ne.0)then !! if the cell isn't empty
       ...
       do jj=is,ie       !! Loop over all particles in cell jc
          ...
          rr2tmp=dot_product(rij,rij)  !! Distance squared
          if(rr2tmp .le. stencilsize2)then              
             ij_count_c(ii)=ij_count_c(ii)+1;              !! Increment count
             ij_link_c(ij_count_c(ii),ii)=j                !! add to list  
          endif          
       end do
    end if
  
    return
  end subroutine neighbour_node_cell_c
!! ------------------------------------------------------------------------------------------------  
  subroutine order_neighbours_c
    !! This routine sorts the neighbour list in order of increasing distance.
    integer(ikind) :: i,j,k
    real(rkind) :: rad2
    real(rkind),dimension(ithree) :: rij
...
!          rij2(k) = dble(j)
       ...
       !! Now sort by value of rij2
       call quicksort_neighbours_c(i,rij2,1,ij_count_c(i))       
       
       !! Build the small ij_count
       ij_count_small_c(i) = 0
       do k=1,ij_count_c(i)
          j=ij_link_c(k,i)
          rij = rp(i,:) - rp(j,:)
          rad2 = dot_product(rij,rij)
          if(rad2.le.(h_c(i)*ss)**two) ij_count_small_c(i) = ij_count_small_c(i) + 1
       end do
    ...
    return
  end subroutine order_neighbours_c
!! ------------------------------------------------------------------------------------------------
  recursive subroutine quicksort_neighbours_c(ii,a, first, last)
    integer(ikind),intent(in) :: ii  !! Index of node whose neighbours we're sorting
    real(rkind) ::  a(*), x, t
    integer(ikind),intent(in) :: first, last  !! Start and end indices of sort
    integer(ikind) i, j,it
    ...
    do
       ...
       if (i >= j) exit
       t = a(i);  a(i) = a(j);  a(j) = t
       it = ij_link_c(i,ii); ij_link_c(i,ii) = ij_link_c(j,ii);ij_link_c(j,ii) = it
       i=i+1
       j=j-1
    end do
    if (first.lt.i-1) call quicksort_neighbours_c(ii,a, first, i-1)
    if (j+1.lt.last)  call quicksort_neighbours_c(ii,a, j+1, last)
    
  end subroutine quicksort_neighbours_c
!! ------------------------------------------------------------------------------------------------    
